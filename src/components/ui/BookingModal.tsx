...
export function BookingModal({ isOpen, onClose }: BookingModalProps) {
    ...
    const handleSubmit = () => {
        ...
        const subject = `New Inquiry from Portfolio: ${selectedDate.toDateString()} @ ${selectedTime}`;
        const body = `Hi Subho,\n\nI'm interested in booking a call.\n\nDate: ${selectedDate.toDateString()}\nTime: ${selectedTime}\n\nProject Details:\n${query}\n\nLooking forward to it.`;

        window.location.href = `mailto:ghoruisubho1999@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
        onClose();
    };
                            ...
                            <div className="p-6 md:p-8 overflow-y-auto">
                                ...
                                {step === 3 && (
                                    <div className="space-y-6">
                                        ...
                                        <div className="flex justify-between mt-8">
                                            <button onClick={() => setStep(2)} className="underline opacity-50 hover:opacity-100">Back</button>
                                            <button
                                                onClick={handleSubmit}
                                                className="bg-neon-lime text-black px-8 py-4 font-display font-bold uppercase hover:scale-105 transition-transform"
                                            >
                                                Send Request
                                            </button>
                                        </div>
